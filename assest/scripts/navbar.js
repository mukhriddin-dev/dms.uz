...
const userImage= document.querySelector('.user-img');
 ...
 const cardImg=document.querySelectorAll('.circle-card__img');
 const trucCar=document.querySelector('.truck-img');
...
if(st>645){
 cardImg.forEach((item )=> item.style.transform='translate(-50%, -50%) rotate(0deg) scale(1.2)');
}else{
  cardImg.forEach((item )=> item.style.transform='translate(-50%, -50%) rotate(-45deg) scale(1)');
}
